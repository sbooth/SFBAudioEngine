...
extension AudioTransportManager {
	...
	func createEndpointDevice(composition: [AnyHashable: Any]) throws -> AudioDevice {
		...
		return AudioObject.make(try getProperty(AudioObjectProperty(kAudioTransportManagerCreateEndPointDevice), qualifier: PropertyQualifier(&qualifier))) as! AudioDevice
	}
	...
	func destroyEndpointDevice(_ endpointDevice: AudioDevice) throws {
		_ = try getProperty(AudioObjectProperty(kAudioTransportManagerDestroyEndPointDevice), initialValue: endpointDevice.objectID)
	}
	...
	public func endpointList() throws -> [AudioObject] {
		return try getProperty(AudioObjectProperty(kAudioTransportManagerPropertyEndPointList)).map { AudioObject.make($0) }
	}
	...
	public func endpoint(_ uid: String) throws -> AudioObject? {
		...
		guard let endpointObjectID: AudioObjectID = try? getProperty(AudioObjectProperty(kAudioTransportManagerPropertyTranslateUIDToEndPoint), qualifier: PropertyQualifier(&qualifierData)), endpointObjectID != kAudioObjectUnknown else {
			return nil
		}
	...
	public func transportType() throws -> AudioDevice.TransportType {
		return AudioDevice.TransportType(rawValue: try getProperty(AudioObjectProperty(kAudioTransportManagerPropertyTransportType)))
	}
}
