...
extension AudioClockDevice {
	...
	public func clockDeviceUID() throws -> String {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyDeviceUID))
	}
	...
	public func transportType() throws -> AudioDevice.TransportType {
		return AudioDevice.TransportType(rawValue: try getProperty(AudioObjectProperty(kAudioClockDevicePropertyTransportType)))
	}
	...
	public func isAlive() throws -> Bool {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyDeviceIsAlive)) != 0
	}

	/// Returns `true` if the clock device is running (`kAudioClockDevicePropertyClockDomain`)
	public func isRunning() throws -> Bool {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyClockDomain)) != 0
	}

	/// Returns the latency (`kAudioClockDevicePropertyDeviceIsRunning`)
	public func latency() throws -> UInt32 {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyDeviceIsRunning))
	}

	/// Returns the audio controls owned by `self` (`kAudioClockDevicePropertyControlList`)
	public func controlList() throws -> [AudioControl] {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyControlList)).map { AudioObject.make($0) as! AudioControl }
	}

	/// Returns the sample rate (`kAudioClockDevicePropertyNominalSampleRate`)
	public func sampleRate() throws -> Double {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyNominalSampleRate))
	}
	...
	public func availableSampleRates() throws -> [AudioValueRange] {
		return try getProperty(AudioObjectProperty(kAudioClockDevicePropertyAvailableNominalSampleRates))
	}
}
