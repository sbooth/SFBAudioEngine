...
public class AudioBox: AudioObject {
	...
	public class func boxes() throws -> [AudioBox] {
		return try AudioSystemObject.instance.getProperty(PropertyAddress(kAudioHardwarePropertyBoxList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioBox }
	}
...
extension AudioBox {
	...
	public func deviceList() throws -> [AudioDevice] {
		return try getProperty(PropertyAddress(kAudioBoxPropertyDeviceList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioDevice }
	}
	...
	public func clockDeviceList() throws -> [AudioClockDevice] {
		return try getProperty(PropertyAddress(kAudioBoxPropertyClockDeviceList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioClockDevice }
	}
}
