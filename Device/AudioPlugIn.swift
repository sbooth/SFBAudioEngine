...
public class AudioPlugIn: AudioObject {
	...
	public class func plugIns() throws -> [AudioPlugIn] {
		return try AudioSystemObject.instance.getProperty(PropertyAddress(kAudioHardwarePropertyPlugInList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioPlugIn }
	}
	...
	public class func makePlugIn(forBundleID bundleID: String) throws -> AudioPlugIn? {
		...
		let objectID = try AudioSystemObject.instance.getProperty(PropertyAddress(kAudioHardwarePropertyTranslateBundleIDToPlugIn), type: AudioObjectID.self, qualifier: PropertyQualifier(&qualifier))
...
extension AudioPlugIn {
	...
	func destroyAggregateDevice(_ aggregateDevice: AudioDevice) throws {
		_ = try getProperty(PropertyAddress(kAudioPlugInDestroyAggregateDevice), type: AudioObjectID.self, initialValue: aggregateDevice.objectID)
	}
	...
	public func deviceList() throws -> [AudioDevice] {
		return try getProperty(PropertyAddress(kAudioPlugInPropertyDeviceList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioDevice }
	}
	...
	public func boxList() throws -> [AudioBox] {
		return try getProperty(PropertyAddress(kAudioPlugInPropertyBoxList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioBox }
	}
	...
	public func clockDeviceList() throws -> [AudioClockDevice] {
		return try getProperty(PropertyAddress(kAudioPlugInPropertyClockDeviceList), arrayType: AudioObjectID.self).map { AudioObject.make($0) as! AudioClockDevice }
	}
