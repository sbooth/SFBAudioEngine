...
extension EndpointDevice {
	...
	func composition() throws -> [AnyHashable: Any] {
		return try dictionaryForProperty(.endpointDeviceComposition)
	}
	...
	func endpoints() throws -> [AudioDevice] {
		return try audioObjectsForProperty(.endpointDeviceEndPointList) as! [AudioDevice]
	}
	...
	func isPrivate() throws -> pid_t {
		return pid_t(try uintForProperty(.endpointDeviceIsPrivate))
	}
}
