...
- (BOOL)openReturningError:(NSError **)error
{
	...
	if(!FLAC__stream_decoder_process_until_end_of_metadata(_flac)) {
		os_log_error(OS_LOG_DEFAULT, "FLAC__stream_decoder_process_until_end_of_metadata failed: %{public}s", FLAC__stream_decoder_get_resolved_state_string(_flac));
	...
	switch(processingStreamDescription.mBitsPerChannel) {
		...
		default: {
			os_log_error(OS_LOG_DEFAULT, "Unsupported bit depth: %u", _streamInfo.bits_per_sample);
