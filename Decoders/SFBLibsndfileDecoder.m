...
- (BOOL)openReturningError:(NSError **)error
{
	...
	if(!_sndfile) {
		os_log_error(OS_LOG_DEFAULT, "sf_open_virtual failed: %{public}s", sf_error_number(sf_error(NULL)));
...
- (BOOL)closeReturningError:(NSError **)error
{
	if(_sndfile) {
		...
		if(result)
			os_log_error(OS_LOG_DEFAULT, "sf_close failed: %{public}s", sf_error_number(result));
		_sndfile = NULL;
	}
...
- (BOOL)decodeIntoBuffer:(AVAudioPCMBuffer *)buffer frameLength:(AVAudioFrameCount)frameLength error:(NSError **)error
{
	...
	if(result) {
		os_log_error(OS_LOG_DEFAULT, "sf_readf_XXX failed: %{public}s", sf_error_number(result));
		return NO;
	}
...
- (BOOL)seekToFrame:(AVAudioFramePosition)frame error:(NSError **)error
{
	...
	if(result == -1) {
		os_log_error(OS_LOG_DEFAULT, "sf_seek failed: %{public}s", sf_error_number(sf_error(_sndfile)));
		return NO;
	}
	return YES;
}
