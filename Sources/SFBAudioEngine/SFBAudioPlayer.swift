...
extension AudioPlayer {
    /// Returns the frame position in the current decoder or `nil` if unknown or the current playback snapshot is invalid
    ...
    /// Returns the frame length of the current decoder or `nil` if unknown or the current playback snapshot is invalid
    ...
    /// Returns the playback position in the current decoder or `nil` if invalid or the current playback snapshot is invalid
    ...
    /// Returns the current time in the current decoder or `nil` if unknown or the current playback snapshot is invalid
    public var currentTime: TimeInterval? {
        let currentTime = __currentTime
        return currentTime == unknownTime ? nil : currentTime
    }

    /// Returns the total time of the current decoder or `nil` if unknown or the current playback snapshot is invalid
    public var totalTime: TimeInterval? {
        let totalTime = __totalTime
        return totalTime == unknownTime ? nil : totalTime
    }

    /// Returns the playback time in the current decoder or `nil` if invalid or the current playback snapshot is invalid
    ...
    /// Returns the playback position and time in the current decoder or `nil` if the current playback snapshot is invalid
