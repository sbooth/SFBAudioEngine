...
inline uint64_t DecoderState::sSequenceNumber = 1;
...
inline DecoderState * _Nullable GetActiveDecoderStateWithSmallestSequenceNumber(const DecoderStateArray& decoders) noexcept
{
...
inline DecoderState * _Nullable GetActiveDecoderStateFollowingSequenceNumber(const DecoderStateArray& decoders, const uint64_t& sequenceNumber) noexcept
{
	DecoderState *result = nullptr;
	for(const auto& atomic_ptr : decoders) {
		auto decoderState = atomic_ptr.load();
...
inline DecoderState * _Nullable GetDecoderStateWithSequenceNumber(const DecoderStateArray& decoders, const uint64_t& sequenceNumber) noexcept
{
	for(const auto& atomic_ptr : decoders) {
		auto decoderState = atomic_ptr.load();
		if(!decoderState)
...
inline void DeleteDecoderStateWithSequenceNumber(DecoderStateArray& decoders, const uint64_t& sequenceNumber) noexcept
{
...
inline uint64_t NextEventIdentificationNumber() noexcept
{
...
inline void release_nserror_f(void * _Null_unspecified context)
{
...
#pragma mark - Queue Management
	...
	{
		auto cancelDecoder = [&](DecoderState *decoderState) {
