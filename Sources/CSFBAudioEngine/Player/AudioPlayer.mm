        ...
        // Get the earliest decoder state that has not completed decoding
        {
            ...
            const auto iter = std::ranges::find_if(activeDecoders_, [](const auto &decoderState) noexcept {
                ...
                return bits::has_none(decoderFlags,
                                      DecoderState::Flags::canceled | DecoderState::Flags::decodingComplete);
            });
...
#if DEBUG
    assert(command == EventCommand::seekRequest);
#endif /* DEBUG */
...
#if DEBUG
            if (!decoderState->decoder_.supportsSeeking) {
                os_log_error(log_, "Seek request received for non-seekable %{public}@", decoderState->decoder_);
                return false;
            }
#endif /* DEBUG */
