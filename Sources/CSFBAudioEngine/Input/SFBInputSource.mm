...
NSError *NSErrorFromInputSourceException(const std::exception *e) noexcept {
    ...
    if (const auto se = dynamic_cast<const std::system_error *>(e); se) {
        return [NSError errorWithDomain:NSPOSIXErrorDomain
                                   code:se->code().value()
                               userInfo:@{
                                   NSDebugDescriptionErrorKey : @(se->code().message().c_str())
                               }];
    }
    ...
    if (const auto ia = dynamic_cast<const std::invalid_argument *>(e); ia) {
        return [NSError errorWithDomain:NSPOSIXErrorDomain
                                   code:EINVAL
                               userInfo:@{
                                   NSDebugDescriptionErrorKey : @(ia->what())
                               }];
    }
    ...
    if (const auto oor = dynamic_cast<const std::out_of_range *>(e); oor) {
        return [NSError errorWithDomain:NSPOSIXErrorDomain
                                   code:EDOM
                               userInfo:@{
                                   NSDebugDescriptionErrorKey : @(oor->what())
                               }];
    }

    return [NSError errorWithDomain:SFBInputSourceErrorDomain
                               code:SFBInputSourceErrorCodeInputOutput
                           userInfo:@{
                               NSDebugDescriptionErrorKey : @(e->what())
                           }];
}
...
+ (void)load {
    [NSError setUserInfoValueProviderForDomain:SFBInputSourceErrorDomain
...
- (BOOL)readInt8:(int8_t *)i8 error:(NSError **)error {
    return [self readUInt8:(uint8_t *)i8 error:error];
}
- (BOOL)readInt16:(int16_t *)i16 error:(NSError **)error {
    return [self readUInt16:(uint16_t *)i16 error:error];
}
- (BOOL)readInt32:(int32_t *)i32 error:(NSError **)error {
    return [self readUInt32:(uint32_t *)i32 error:error];
}
- (BOOL)readInt64:(int64_t *)i64 error:(NSError **)error {
    return [self readUInt64:(uint64_t *)i64 error:error];
}
@end
