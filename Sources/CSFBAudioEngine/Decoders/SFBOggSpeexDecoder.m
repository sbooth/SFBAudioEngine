...
- (BOOL)openReturningError:(NSError **)error {
            ...
            *error = SFBErrorWithLocalizedDescription(
                  ...
                  NSLocalizedString(@"The file “%@” is not a supported Ogg Speex file.", @""), @{
                      NSLocalizedRecoverySuggestionErrorKey : [NSString
                            stringWithFormat:NSLocalizedString(@"Speex mode %d is not supported.", @""), header->mode],
                      NSURLErrorKey : _inputSource.url
                  },
                  SFBLocalizedNameForURL(_inputSource.url));
        }
    ...
    if (mode->bitstream_version != header->mode_bitstream_version) {
        ...
        if (error) {
            *error = SFBErrorWithLocalizedDescription(
                  ...
                  NSLocalizedString(@"The file “%@” is not a supported Ogg Speex file.", @""), @{
                      NSLocalizedRecoverySuggestionErrorKey : [NSString
                            stringWithFormat:NSLocalizedString(@"Speex bitstream version %d is not supported.", @""),
    ...
    _processingFormat = [[AVAudioFormat alloc] initWithCommonFormat:AVAudioPCMFormatFloat32
                                                         sampleRate:header->rate
                                                           channels:(AVAudioChannelCount)header->nb_channels
                                                        interleaved:YES];
    ...
    _properties = @{
        SFBAudioDecodingPropertiesKeyOggSpeexSpeexString : [[NSString alloc] initWithBytes:header->speex_string
                                                                                    length:SPEEX_HEADER_STRING_LENGTH
                                                                                  encoding:NSASCIIStringEncoding],
        SFBAudioDecodingPropertiesKeyOggSpeexSpeexVersion : [[NSString alloc] initWithBytes:header->speex_version
                                                                                     length:SPEEX_HEADER_VERSION_LENGTH
                                                                                   encoding:NSASCIIStringEncoding],
                ...
                if (result == 1) {
                    ...
                    if (_oggPacketCount != -1 && _extraSpeexHeaderCount + 1 <= _oggPacketCount) {
                        ...
                        for (spx_int32_t i = 0; i < _framesPerOggPacket; ++i) {
                            ...
                            // Copy the frames from the decoding buffer to the output buffer, skipping over any frames
                            // already decoded

                            float *output = _buffer.floatChannelData[0] + (_buffer.frameLength * _buffer.stride);
                            memcpy(output, buf,
                                   (size_t)speexFrameSize * _processingFormat.channelCount * sizeof(float));
