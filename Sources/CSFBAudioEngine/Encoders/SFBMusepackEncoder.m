...
static size_t my_mpc_write_callback(const void *restrict ptr, size_t size, size_t nitems, void *context) {
    ...
    if (![encoder->_outputWriter writeBytes:ptr
...
static int my_mpc_seek_callback(void *context, off_t offset, int whence) {
    ...
    case SEEK_SET:
        ...
        break;
    case SEEK_CUR: {
        NSInteger outputWriterOffset;
        if ([encoder->_outputWriter getOffset:&outputWriterOffset error:nil]) {
            offset += outputWriterOffset;
        }
        break;
    }
    case SEEK_END: {
        NSInteger outputWriterLength;
        if ([encoder->_outputWriter getLength:&outputWriterLength error:nil]) {
            offset += outputWriterLength;
        }
        break;
    }
    }
