...
- (BOOL)encodeFromBuffer:(AVAudioPCMBuffer *)buffer frameLength:(AVAudioFrameCount)frameLength error:(NSError **)error {
    ...
    if (![_outputWriter writeBytes:buf.get() length:result bytesWritten:&bytesWritten error:error] ||
        bytesWritten != result) {
        return NO;
    }
...
- (BOOL)writeID3v1TagReturningError:(NSError **)error {
    ...
    if (bufsize > 0) {
        ...
        NSInteger bytesWritten;
        if (![_outputWriter writeBytes:buf.get()
...
- (BOOL)writeXingHeaderReturningError:(NSError **)error {
    ...
    if (bufsize > 0) {
        ...
        if (![_outputWriter seekToOffset:_id3v2TagSize error:error]) {
            return NO;
        }
