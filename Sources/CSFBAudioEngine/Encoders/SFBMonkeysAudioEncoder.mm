...
class APEIOInterface final : public APE::IAPEIO {
  public:
    explicit APEIOInterface(SFBOutputWriter *outputWriter) : outputWriter_(outputWriter) {}
...
#pragma unused(bOpenReadOnly)
    ...
    int Read(void *pBuffer, unsigned int nBytesToRead, unsigned int *pBytesRead) override {
        NSInteger bytesRead;
        if (![outputWriter_ readBytes:pBuffer length:nBytesToRead bytesRead:&bytesRead error:nil]) {
    ...
    int Write(const void *pBuffer, unsigned int nBytesToWrite, unsigned int *pBytesWritten) override {
        ...
        if (![outputWriter_ writeBytes:pBuffer length:(NSInteger)nBytesToWrite bytesWritten:&bytesWritten error:nil] ||
    ...
    int Seek(APE::int64 nPosition, APE::SeekMethod nMethod) override {
        if (!outputWriter_.supportsSeeking) {
            return ERROR_IO_READ;
        }
        ...
        case APE::SeekFileCurrent:
            if (NSInteger inputSourceOffset; [outputWriter_ getOffset:&inputSourceOffset error:nil]) {
                offset += inputSourceOffset;
            }
            break;
        case APE::SeekFileEnd:
            if (NSInteger inputSourceLength; [outputWriter_ getLength:&inputSourceLength error:nil]) {
                offset += inputSourceLength;
            }
            break;
        }

        if (![outputWriter_ seekToOffset:offset error:nil]) {
            return ERROR_IO_READ;
        }
...
#pragma unused(pnBufferBytes)
    ...
    APE::int64 GetPosition() override {
        NSInteger offset;
        if (![outputWriter_ getOffset:&offset error:nil]) {
            return -1;
        }
        return offset;
    }
    ...
    APE::int64 GetSize() override {
        NSInteger length;
        if (![outputWriter_ getLength:&length error:nil]) {
            return -1;
        }
        return length;
    }
...
- (BOOL)openReturningError:(NSError **)error {
    ...
    try {
        ...
        _ioInterface = std::make_unique<APEIOInterface>(_outputWriter);
