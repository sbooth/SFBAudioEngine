...
static int wavpack_block_output(void *id, void *data, int32_t bcount) {
    ...
    return [encoder->_outputWriter writeBytes:data length:bcount bytesWritten:&bytesWritten error:nil];
}
...
#pragma clang diagnostic pop
    ...
    if (_estimatedFramesToEncode != _framePosition && _firstBlock) {
        ...
        if (![_outputWriter seekToOffset:0 error:error]) {
            return NO;
        }
        if (![_outputWriter writeData:_firstBlock error:error]) {
            return NO;
        }
    }
