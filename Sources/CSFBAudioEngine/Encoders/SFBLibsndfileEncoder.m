...
static sf_count_t my_sf_vio_get_filelen(void *user_data) {
    ...
    NSInteger length;
    if (![encoder->_outputWriter getLength:&length error:nil]) {
        return -1;
    }
    return length;
}
...
static sf_count_t my_sf_vio_seek(sf_count_t offset, int whence, void *user_data) {
    ...
    SFBLibsndfileEncoder *encoder = (__bridge SFBLibsndfileEncoder *)user_data;
    if (!encoder->_outputWriter.supportsSeeking) {
        return -1;
    }
    ...
    case SEEK_CUR: {
        ...
        if ([encoder->_outputWriter getOffset:&inputSourceOffset error:nil]) {
    ...
    case SEEK_END: {
        ...
        if ([encoder->_outputWriter getLength:&inputSourceLength error:nil]) {
    ...
    if (![encoder->_outputWriter seekToOffset:offset error:nil]) {
        return -1;
    }
...
static sf_count_t my_sf_vio_read(void *ptr, sf_count_t count, void *user_data) {
    ...
    NSInteger bytesRead;
    if (![encoder->_outputWriter readBytes:ptr length:count bytesRead:&bytesRead error:nil]) {
        return -1;
    }
    return bytesRead;
}
...
static sf_count_t my_sf_vio_write(const void *ptr, sf_count_t count, void *user_data) {
    ...
    if (![encoder->_outputWriter writeBytes:ptr length:(NSInteger)count bytesWritten:&bytesWritten error:nil]) {
        return 0;
    }
...
static sf_count_t my_sf_vio_tell(void *user_data) {
    ...
    SFBLibsndfileEncoder *encoder = (__bridge SFBLibsndfileEncoder *)user_data;
    NSInteger offset;
    if (![encoder->_outputWriter getOffset:&offset error:nil]) {
        return -1;
    }
    return offset;
}
...
- (BOOL)openReturningError:(NSError **)error {
    ...
    if (majorFormatSetting) {
        ...
        majorFormat = MajorFormatForExtension(_outputWriter.url.pathExtension);
        os_log_info(gSFBAudioEncoderLog,
                    ...
                    "\"%{public}@\"",
                    majorFormat, _outputWriter.url.pathExtension);
    }
