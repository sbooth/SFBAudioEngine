...
TTAint32 writeCallback(struct _tag_TTA_io_callback *io, TTAuint8 *buffer, TTAuint32 size) noexcept {
    ...
    if (![iocb->encoder_->_outputWriter writeBytes:buffer length:size bytesWritten:&bytesWritten error:nil]) {
        return -1;
    }
...
TTAint64 seekCallback(struct _tag_TTA_io_callback *io, TTAint64 offset) noexcept {
    ...
    if (![iocb->encoder_->_outputWriter seekToOffset:offset error:nil]) {
        return -1;
    }
    return offset;
}
