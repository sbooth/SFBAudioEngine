...
+ (BOOL)convertFromURL:(NSURL *)sourceURL
                 toURL:(NSURL *)destinationURL
                 error:(NSError **)error NS_SWIFT_NAME(AudioConverter.convert(_:to:));
...
+ (BOOL)convertFromURL:(NSURL *)sourceURL
          usingEncoder:(id<SFBPCMEncoding>)encoder
                 error:(NSError **)error NS_SWIFT_NAME(AudioConverter.convert(_:using:));
...
/// - parameter decoder: The decoder
/// - parameter encoder: The encoder
/// - parameter intermediateFormatBlock: An optional block to receive the proposed intermediate format and return the
/// requested intermediate format. A change in intermediate format allows operations such as sample rate conversion or
/// channel mapping.
/// - parameter error: An optional pointer to an `NSError` object to receive error information
/// - returns: An initialized `SFBAudioConverter` object for the specified decoder and encoder, or `nil` on failure
- (nullable instancetype)initWithDecoder:(id<SFBPCMDecoding>)decoder
                                 encoder:(id<SFBPCMEncoding>)encoder
             requestedIntermediateFormat:(AVAudioFormat * (^_Nullable)(AVAudioFormat *))intermediateFormatBlock
