...
@interface SFBFileOutputWriter () {
  @private
    FILE *_file;
}
@end

@implementation SFBFileOutputWriter

- (BOOL)openReturningError:(NSError **)error {
    _file = fopen(_url.fileSystemRepresentation, "w+");
    if (!_file) {
        int err = errno;
        os_log_error(gSFBOutputWriterLog, "fopen failed: %{public}s (%d)", strerror(err), err);
        if (error) {
...
- (BOOL)closeReturningError:(NSError **)error {
    if (_file) {
        ...
        if (result) {
            int err = errno;
            os_log_error(gSFBOutputWriterLog, "fclose failed: %{public}s (%d)", strerror(err), err);
            if (error) {
                *error = [self posixErrorWithCode:err];
            }
            return NO;
...
- (BOOL)readBytes:(void *)buffer length:(NSInteger)length bytesRead:(NSInteger *)bytesRead error:(NSError **)error {
    ...
    if (read != (size_t)length && ferror(_file)) {
        int err = errno;
        os_log_error(gSFBOutputWriterLog, "fread error: %{public}s (%d)", strerror(err), err);
        if (error) {
            *error = [self posixErrorWithCode:err];
        }
        return NO;
...
- (BOOL)writeBytes:(const void *)buffer
    ...
    if (written != (size_t)length) {
        int err = errno;
        os_log_error(gSFBOutputWriterLog, "fwrite error: %{public}s (%d)", strerror(err), err);
        if (error) {
            *error = [self posixErrorWithCode:err];
        }
        return NO;
...
- (BOOL)getOffset:(NSInteger *)offset error:(NSError **)error {
    ...
    if (result == -1) {
        int err = errno;
        os_log_error(gSFBOutputWriterLog, "ftello failed: %{public}s (%d)", strerror(err), err);
        if (error) {
            *error = [self posixErrorWithCode:err];
        }
        return NO;
...
- (BOOL)getLength:(NSInteger *)length error:(NSError **)error {
    ...
    if (fseeko(_file, 0, SEEK_END)) {
        int err = errno;
        os_log_error(gSFBOutputWriterLog, "fseeko(0, SEEK_END) error: %{public}s (%d)", strerror(err), err);
        if (error) {
            *error = [self posixErrorWithCode:err];
        }
        return NO;
    ...
    if (fseeko(_file, offset, SEEK_SET)) {
        int err = errno;
        os_log_error(gSFBOutputWriterLog, "fseeko(%ld, SEEK_SET) error: %{public}s (%d)", (long)offset, strerror(err),
                     err);
        if (error) {
            *error = [self posixErrorWithCode:err];
        }
